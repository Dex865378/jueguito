...
import { 
  ...
  Terminal, Grid, Key, AlertTriangle, Cpu, CheckCircle
} from 'lucide-react';
...
let ambianceOsc = null;
let ambianceGain = null;
let lfo = null;

const toggleAmbianceAudio = (enable) => {
  if (enable && !ambianceOsc) {
    const ctx = new (window.AudioContext || window.webkitAudioContext)();
    ambianceOsc = ctx.createOscillator();
    ambianceGain = ctx.createGain();
    
    ambianceOsc.connect(ambianceGain);
    ambianceGain.connect(ctx.destination);
    
    ambianceOsc.type = 'sawtooth';
    ambianceOsc.frequency.setValueAtTime(55, ctx.currentTime); // Deep drone
    ambianceGain.gain.setValueAtTime(0, ctx.currentTime);
    ambianceGain.gain.linearRampToValueAtTime(0.02, ctx.currentTime + 2);
    
    // Add LFO for tension/wobble
    ...
    lfo.connect(lfoGain);
    lfoGain.connect(ambianceOsc.frequency);
    
    ambianceOsc.start();
    lfo.start();
  } else if (!enable && ambianceOsc) {
    ambianceGain.gain.exponentialRampToValueAtTime(0.0001, ambianceOsc.context.currentTime + 1);
    setTimeout(() => {
      if (ambianceOsc) ambianceOsc.stop();
      if (lfo) lfo.stop();
      ambianceOsc = null;
      ambianceGain = null;
      lfo = null;
    ...
    <div className="flex flex-col items-center gap-10 w-full max-w-lg">
      <div className="relative w-full h-48 bg-black/40 border border-white/10 rounded-3xl overflow-hidden flex items-center shadow-[inset_0_0_20px_rgba(0,0,0,0.5)]">
        ...
        <div className="absolute inset-0 w-full flex justify-between px-4 items-center opacity-20 pointer-events-none">
           {Array.from({length: 40}).map((_, i) => {
             const t = i / 40;
             const h = 10 + config.targetAmp * 0.8 * Math.abs(Math.sin(t * config.targetFreq * Math.PI));
             return <div key={i} className="w-1.5 bg-emerald-500 rounded-full" style={{ height: `${h}%` }} />
           })}
        </div>
        ...
        <div className="relative w-full flex justify-between px-4 items-center">
           {Array.from({length: 40}).map((_, i) => {
             const t = i / 40;
             const h = 10 + amp * 0.8 * Math.abs(Math.sin(t * freq * Math.PI));
             return <div key={i} className="w-1.5 bg-indigo-500 rounded-full shadow-[0_0_10px_#6366f1] transition-all duration-75" style={{ height: `${h}%` }} />
           })}
        </div>
        
        <div className="absolute inset-x-0 bottom-0 h-px bg-indigo-500/30" />
      </div>
      ...
      <div className="w-full grid grid-cols-2 gap-8 bg-zinc-900/40 p-6 rounded-3xl border border-white/5">
        <div className="flex flex-col gap-3">
          ...
          <input type="range" min="1" max="10" value={freq} onChange={(e) => {playSound('click'); setFreq(parseInt(e.target.value));}} className="w-full h-2 rounded-lg appearance-none cursor-pointer bg-white/10 accent-indigo-500"/>
        </div>
        <div className="flex flex-col gap-3">
          <div className="flex justify-between text-[10px] font-black tracking-widest text-zinc-500">
            <span>AMPLITUD</span> <span className="text-white">{amp}</span>
          </div>
          <input type="range" min="10" max="100" value={amp} onChange={(e) => {playSound('click'); setAmp(parseInt(e.target.value));}} className="w-full h-2 rounded-lg appearance-none cursor-pointer bg-white/10 accent-purple-500"/>
        </div>
      </div>
    </div>
  );
...
function App() {
  ...
  return (
    ...
    <div className={`min-h-screen ${isHardcore ? 'bg-zinc-900 crt-screen' : 'bg-zinc-950'} text-zinc-100 p-4 md:p-10 selection:bg-indigo-500/30 overflow-y-auto relative no-select transition-colors`}>
      {/* Background Effect */}
      <div className="fixed inset-0 cyber-grid opacity-[0.15]" />
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        <header className="flex flex-col lg:flex-row justify-between items-center mb-8 lg:mb-16 gap-6 glass-card p-6 lg:p-8 rounded-[2rem] lg:rounded-[2.5rem]">
          ...
          <div className="flex flex-wrap items-center justify-between w-full lg:w-auto gap-4 lg:gap-8 border-t lg:border-t-0 border-white/5 pt-4 lg:pt-0">
            {/* New modifiers toggles */}
            <div className="flex gap-2">
               <button onClick={() => {
                 const newState = !ambianceActive;
                 setAmbianceActive(newState);
                 toggleAmbianceAudio(newState);
               }} className={`px-4 py-2 text-[8px] font-black rounded-xl border transition-all ${ambianceActive ? 'bg-indigo-600 border-indigo-500 shadow-xl text-white' : 'bg-white/5 border-white/10 text-zinc-600'}`}>MÚSICA</button>
               
               <button onClick={() => setIsHardcore(!isHardcore)} className={`px-4 py-2 text-[8px] font-black rounded-xl border transition-all ${isHardcore ? 'bg-red-600 border-red-500 shadow-xl text-white animate-pulse' : 'bg-white/5 border-white/10 text-zinc-600'}`}>CRT_GLITCH</button>
            ...
            <StatBox label={gameMode === 'survival' ? 'SURVIVAL_SCORE' : 'SCORE'} value={gameMode === 'survival' ? survivalState.score : score} color="text-indigo-400" />
            <StatBox label="NODES" value={`${playerAchievements.length}/${ACHIEVEMENTS.length}`} color="text-emerald-400" />
          ...
          <aside className={`lg:col-span-3 space-y-8 lg:space-y-12 ${sidebarOpen ? 'block' : 'hidden lg:block'}`}>
            <section>
              <div className="flex gap-4 mb-8">
                <button onClick={() => setActiveTab('critical')} className={`flex-1 py-3 text-[8px] font-black rounded-xl border transition-all ${activeTab === 'critical' ? 'bg-indigo-600 border-indigo-500 shadow-xl' : 'bg-white/5 border-white/10 text-zinc-600'}`}>CRÍTICO</button>
                <button onClick={() => setActiveTab('fun')} className={`flex-1 py-3 text-[8px] font-black rounded-xl border transition-all ${activeTab === 'fun' ? 'bg-emerald-600 border-emerald-500 shadow-xl' : 'bg-white/5 border-white/10 text-zinc-600'}`}>DIVERSIÓN</button>
              </div>

              <h3 className="text-[10px] font-black text-zinc-600 uppercase tracking-widest mb-6 lg:mb-8 flex items-center gap-3">
                <Grid className="w-3 h-3" /> ESTRUCTURA_DE_DATOS
              </h3>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-1 gap-3 lg:gap-4 overflow-y-auto max-h-[500px] pr-2 custom-scrollbar">
                {PUZZLES.filter(p => activeTab === 'critical' ? (p.category !== 'recreational') : (p.category === 'recreational')).map((p) => (
                  <LevelTab 
                    key={p.id}
                    title={p.title}
                    id={p.id}
                    active={activePuzzle.id === p.id}
                    locked={!unlockedLevels.includes(PUZZLES.indexOf(p))}
                    solved={solvedLevels.includes(PUZZLES.indexOf(p))}
                    onClick={() => { 
                      setCurrentLevel(PUZZLES.indexOf(p)); 
                      setIsReplaying(false);
            ...
            <AnimatePresence mode="wait">
              ...
              >
                <motion.div
                  ...
                  className="glass-card rounded-[4rem] p-10 md:p-16 relative overflow-hidden min-h-[650px] flex flex-col"
                >
                <div className="relative z-10 flex-1 flex flex-col">
                  ...
                  <h2 className={`text-4xl md:text-6xl font-black mb-6 tracking-tighter leading-none italic ${isAnomaly || isHardcore ? 'hardcore-glitch text-white' : 'text-glow'} ${isHardcore && !isAnomaly && 'text-red-200'} `}>
                    {isAnomaly ? 'ERR_BUFFER_OVERFLOW' : activePuzzle.title}
                  </h2>
                  <p className={`text-zinc-400 text-xl max-w-2xl leading-relaxed mb-16 ${isHardcore && 'uppercase tracking-widest font-mono text-[10px] text-zinc-500'}`}>
                    {isAnomaly ? 'Fallo en la matriz de aislamiento. Restableciendo conexión remota...' : activePuzzle.description}
                  </p>

                  <div className={`flex-1 bg-white/[0.02] border rounded-[2rem] lg:rounded-[3rem] flex flex-col items-center justify-center p-4 lg:p-12 relative overflow-hidden transition-all duration-300 ${isAnomaly ? 'border-red-500/50 bg-red-500/5 shake-heavy' : 'border-white/5'}`}>
                    <AnimatePresence mode="wait">
                      {!showSuccess ? (
                        <motion.div 
                          key="game"
                        ...
                        >
                          <div className="w-32 h-32 bg-emerald-500/10 rounded-full flex items-center justify-center mx-auto mb-10 ring-1 ring-emerald-500/40">
                            <Trophy className="w-16 h-16 text-emerald-400 shadow-[0_0_30px_rgba(52,211,153,0.5)]" />
                          </div>
                          <h3 className="text-5xl font-black text-emerald-400 mb-6 tracking-tighter">ACCESO_PERMITIDO</h3>
                          <p className="text-zinc-600 text-lg mb-12 font-bold uppercase tracking-widest italic">Signal integrity: 100%</p>
                          
                          <div className="flex flex-col sm:flex-row gap-4 justify-center">
                            <button 
                              onClick={() => setIsReplaying(true)}
                              className="px-10 py-5 bg-white/5 hover:bg-white/10 border border-white/10 rounded-3xl font-black text-[10px] tracking-[0.3em] transition-all uppercase"
                            >
                              Volver a jugar
                            </button>
                            {nextIdxToPlay !== -1 && (
                              <button 
                                ...
                                className="group cyber-btn-hover inline-flex items-center gap-4 px-10 py-5 bg-indigo-600 hover:bg-indigo-500 border border-indigo-400/50 rounded-3xl font-black text-[10px] tracking-[0.3em] transition-all shadow-[0_10px_30px_rgba(99,102,241,0.3)] uppercase text-white"
                              >
                                Siguiente <ChevronRight className="w-5 h-5 group-hover:translate-x-2 transition-transform" />
                              </button>
                            )}
                          </div>
                        </motion.div>
...
const Badge = ({ text, color = "text-zinc-500" }) => (
  <span className={`px-5 py-2 bg-white/5 border border-white/10 rounded-2xl text-[10px] font-black uppercase tracking-tighter ${color} flex items-center gap-2`}>
    <div className={`w-1.5 h-1.5 rounded-full bg-current`} /> {text}
  </span>
);
...
const LevelTab = ({ title, id, active, locked, solved, onClick }) => (
  <button
    onClick={onClick}
    disabled={locked}
    className={`w-full p-6 rounded-[2rem] flex items-center justify-between border transition-all duration-500 ${
      active 
      ? 'glass-card border-indigo-500/50 shadow-xl' 
      : 'bg-white/[0.01] border-transparent hover:bg-white/5'
    } ${locked ? 'opacity-20 grayscale cursor-not-allowed' : 'cursor-pointer'}`}
  >
    <div className="flex items-center gap-6">
      <div className={`w-14 h-14 rounded-2xl flex items-center justify-center transition-all ${
        active ? 'bg-indigo-600 shadow-[0_5px_20px_rgba(99,102,241,0.5)]' : 'bg-zinc-900 text-zinc-700'
      }`}>
        {locked ? <Lock className="w-5 h-5" /> : active ? <Brain className="w-7 h-7 text-white" /> : <Shield className="w-6 h-6" />}
      </div>
      <div className="text-left">
        <p className="text-[10px] font-black text-zinc-600 uppercase tracking-widest mb-1">STREAM_0{id}</p>
        <p className={`font-bold text-sm ${active ? 'text-white text-glow' : 'text-zinc-600'}`}>{title}</p>
      </div>
    </div>
    {solved && <CheckCircle className="w-5 h-5 text-emerald-500 shadow-[0_0_10px_rgba(16,185,129,0.5)]" />}
  </button>
);
